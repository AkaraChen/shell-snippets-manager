...
export function SnippetForm({
	...
	const [shellType, setShellType] = useState(
		initialData?.shell_type ?? default_shell,
	);
	...
	const [pendingTemplate, setPendingTemplate] = useState<
		keyof typeof SNIPPET_TEMPLATES | null
	>(null);
	...
	const handleTemplateSelect = (
		templateKey: keyof typeof SNIPPET_TEMPLATES,
	) => {
		if (content.trim()) {
				...
				<div className="flex items-center justify-between">
					...
					<DropdownMenu>
						...
						<DropdownMenuContent
							...
							className="font-[var(--font-mono)]"
						>
							{Object.entries(SNIPPET_TEMPLATES).map(
								([key, template]) => (
									<DropdownMenuItem
										key={key}
										onClick={() =>
											handleTemplateSelect(
												key as keyof typeof SNIPPET_TEMPLATES,
											)
										}
									>
										{template.label}
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This will replace your current content with the
