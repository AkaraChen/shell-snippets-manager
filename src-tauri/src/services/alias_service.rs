...
use crate::models::{
	Alias, AliasResponse, NewAlias, NewSnippet, NewSnippetAlias, Snippet,
	SnippetResponse, UpdateAlias,
};
...
pub fn get_alias_by_id(
	conn: &mut SqliteConnection,
	alias_id: i32,
) -> AppResult<AliasResponse> {
	let alias: Alias =
		aliases.find(alias_id).first(conn).map_err(|e| match e {
			diesel::result::Error::NotFound => {
				AppError::AliasNotFound(alias_id)
			}
			_ => AppError::from(e),
		})?;
...
pub fn add_snippet_to_alias(
	...
	// Verify both exist
	aliases.find(target_alias_id).first::<Alias>(conn).map_err(
		|e| match e {
			diesel::result::Error::NotFound => {
				AppError::AliasNotFound(target_alias_id)
			}
			_ => AppError::from(e),
		},
	)?;
	...
	sn::snippets
		...
		.map_err(|e| match e {
			diesel::result::Error::NotFound => {
				AppError::SnippetNotFound(target_snippet_id)
			}
			_ => AppError::from(e),
		})?;
...
mod tests {
	...
	fn create_test_snippet(
		...
		snippet_service::create_snippet(conn, new)
			.expect("Failed to create test snippet")
	}
