...
pub fn get_snippet_by_id(
	...
	let snippet: Snippet =
		snippets.find(snippet_id).first(conn).map_err(|e| match e {
			diesel::result::Error::NotFound => {
				AppError::SnippetNotFound(snippet_id)
			}
			_ => AppError::from(e),
		})?;
...
mod tests {
	...
	fn create_test_snippet(
		...
		create_snippet(conn, new_snippet)
	...
	fn test_get_enabled_snippets_by_shell() {
		...
		let bash_snippets =
			get_enabled_snippets_by_shell(&mut conn, "bash").unwrap();
		assert_eq!(bash_snippets.len(), 2);

		// Get enabled zsh snippets
		let zsh_snippets =
			get_enabled_snippets_by_shell(&mut conn, "zsh").unwrap();
		assert_eq!(zsh_snippets.len(), 1);

		// Get fish snippets (none)
		let fish_snippets =
			get_enabled_snippets_by_shell(&mut conn, "fish").unwrap();
