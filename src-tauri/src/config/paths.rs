...
impl AppPaths {
	...
	fn initialize_dirs() -> AppResult<(PathBuf, PathBuf)> {
		let strategy = Xdg::new().map_err(|e| {
			AppError::PathError(format!(
				"Failed to initialize XDG strategy: {}",
				e
			))
		})?;

		let data_dir = strategy.data_dir().join("shell-snippets-manager");

		let config_dir = strategy.config_dir().join("shell-snippets-manager");
	...
	fn initialize_dirs() -> AppResult<(PathBuf, PathBuf)> {
		let strategy = Apple::new().map_err(|e| {
			AppError::PathError(format!(
				"Failed to initialize Apple strategy: {}",
				e
			))
		})?;
		...
		let app_dir =
			strategy.data_dir().join("com.akrc.shell-snippets-manager");
...
mod tests {
	...
	fn test_single_dir_macos() {
		...
		assert!(
			paths
				.data_dir()
				.ends_with("com.akrc.shell-snippets-manager"),
